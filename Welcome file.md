...
cor['Cor'] = cor['cor'].abs()
cor = cor.sort_values(by='Cor', ascending=False).drop(columns=['Cor'])
...
file1 <- "./data/data_RNAseq.txt"
file2 <- "./data/data_surival.txt"
rna_data <- read.table(file1, header=TRUE,sep="\t",row.names=1)
surv_data <- read.table(file2, header=TRUE, sep="\t", row.names=1)
rna_data <- t(rna_data)
...
qq_plot <- ggplot(rna_long, aes(sample=E, color=G)) + 
...
print(ks_result)
qqplot(surv_data$OS.time, surv_data$DFI.time,
		main="OS.time vs DFI.time的QQ图比较",
